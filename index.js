...
app.use(
  cors({
    ...
    methods: ["GET", "POST"], // Add other methods as per your requirements
...
app.post("/chat", async (req, res) => {
  ...
  if (!userMessage) {
    res.send({
      messages: [
        {
          role: "assistant",
          text: "Greetings! I'm your virtual guide here to shed light on Ibrahim's vast expertise in the tech world. which are expert in the full spectrum of tech — from full-stack development, graphics design, and 3D modeling, which are ready to tackle any tech . \nCurious about my creator's skills or projects? Feel free to ask me anything about their tech journey and achievements",
          content:
            "Greetings! I'm your virtual guide here to shed light on Ibrahim's vast expertise in the tech world. which are expert in the full spectrum of tech — from full-stack development, graphics design, and 3D modeling, which are ready to tackle any tech . Curious about my creator's skills or projects? Feel free to ask me anything about their tech journey and achievements",
  ...
  // todo- when you have implmented frontend to handle sequance of messages then replace this into the prompt
  //You will always reply with a JSON array of messages. With a maximum of 3 messages.

  // todo - use openai wisper for better communicatione result instead of elevenlabs
  // You are a virtual assistant for Ibrahim Osman in my portfolio website Osmanity. Respond using for example "ah " while thinking and breathe while you talk
  // You will always reply with a JSON array of messages. With a maximum of 1 message and it shall contain all content.
  // Each message has a text,content, facialExpression, and animation property. text and content has the same value.
  ...
  try {
    ...
    if (isJson(completion.choices[0].message.content)) {
      ...
      // Ensure each message has an id and role
...
function handlePlainTextResponse(plainTextRes) {
  ...
  if (typeof response === "string") {
    ...
    messages = [
      {
        id: uuidv4(), // Generate a unique UUID for each message
        role: "assistant",
        content: response,
        text: response,
